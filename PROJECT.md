...
```mermaid
graph TD
  Browser[Client Browser\nSSR/ISR + client interactivity] -->|Page + data| App[Next.js App Router]
...
- Cache headers: `s-maxage=60, stale-while-revalidate=120` for CDN/edge; aligns with ISR.
...
- ISR (60s) + layered caching: better TTFB and upstream protection; data can be ~1 minute stale.
...
- Middleware sets CSP, X-Frame-Options, X-Content-Type-Options.
- Basic input validation on addresses; CID access constrained by cached allowlist/fail-fast fetch.
- No rate limiting yet (see Future Work).
...
1) Add rate limiting on BFF routes (prevent abuse).
2) Add cache-behavior tests (save/read/expiry) and API route contract tests.
3) Skeleton loaders and error boundaries for smoother UX.
4) Pagination or infinite scroll for large token/NFT sets.
5) Analytics + observability (Sentry/logging, metrics).
